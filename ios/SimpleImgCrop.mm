                  ...
                  reject:(RCTPromiseRejectBlock)reject)
{
  ...
  [[self.bridge moduleForName:@"ImageLoader" lazilyLoadIfNecessary:YES] loadImageWithURLRequest:imageRequest callback:^(NSError *error, UIImage *image) {
    if (error) {
    ...
    if([extension isEqualToString:@"png"]){
      ...
      path = [RNCImageUtils generatePathInDirectory:[[RNCImageUtils cacheDirectoryPath] stringByAppendingPathComponent:@"ReactNative_cropped_image_"] withExtension:@".png"];
    }else{
      imageData = UIImageJPEGRepresentation(croppedImage, 1);
      path = [RNCImageUtils generatePathInDirectory:[[RNCImageUtils cacheDirectoryPath] stringByAppendingPathComponent:@"ReactNative_cropped_image_"] withExtension:@".jpg"];
    }
